...
mod tests {
    ...
    fn is_diagonal_works() {
        ...
        let sq_2 = Square::new_without_piece(3, 3);
        assert!(is_diagonal(&sq_1, &sq_2));
        let sq_1 = Square::new_without_piece(1, 0);
        let sq_2 = Square::new_without_piece(4, 3);
        assert!(is_diagonal(&sq_1, &sq_2));
        let sq_1 = Square::new_without_piece(0, 1);
        let sq_2 = Square::new_without_piece(4, 3);
        assert!(!is_diagonal(&sq_1, &sq_2));
        let sq_1 = Square::new_without_piece(0, 7);
        let sq_2 = Square::new_without_piece(7, 0);
        assert!(is_diagonal(&sq_1, &sq_2));
    }
}
