		...
		{#each boardToFront as row, i}
			{#each row as sq, j}
				{#if chess.white_player.in_check && typeof sq.piece === 'object' && sq.piece.King !== undefined && sq.piece.King === 'White'}
					<button
						class="h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 lg:h-22 lg:w-22 xl:w-24 xl:h-24 bg-red-800 text-center hover:bg-red-900"
				...
				{:else if chess.black_player.in_check && typeof sq.piece === 'object' && sq.piece.King !== undefined && sq.piece.King === 'Black'}
					<button
						class="h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 lg:h-22 lg:w-22 xl:w-24 xl:h-24 bg-red-800 hover:bg-red-900 text-center"
						class:selected={selectedButton ===
							sq.file.toLowerCase() + (sq.rank + 1)}
						draggable="true"
						on:dragstart={() => handleDragStart(sq)}
				...
				{:else if sq.color === 'White' && sq.piece !== 'None'}
					<button
						class="h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 lg:h-22 lg:w-22 xl:w-24 xl:h-24 bg-gray-200 text-center hover:bg-gray-600"
						class:selected={selectedButton ===
							sq.file.toLowerCase() + (sq.rank + 1)}
				...
				{:else if sq.color === 'Black' && sq.piece !== 'None'}
					<button
						class="h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 lg:h-22 lg:w-22 xl:w-24 xl:h-24 bg-gray-400 text-center hover:bg-gray-600"
						class:selected={selectedButton ===
							sq.file.toLowerCase() + (sq.rank + 1)}
						class:possible_move={possibleMoves.some(
							(move) => move[1][0] === j && move[1][1] === 8 - i
